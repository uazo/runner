...
namespace GitHub.Runner.Common.Tests.Worker
{
    ...
    public sealed class ContainerOperationProviderL0
    {
        ...
        public async void RunServiceContainersHealthcheck_UnhealthyServiceContainer_AssertFailedTask()
        {
            //Arrange
            Setup();
            _dockerManager.Setup(x => x.DockerInspect(_ec.Object, It.IsAny<string>(), It.IsAny<string>(), true)).Returns(Task.FromResult(unhealthyDockerStatus));
        ...
        public async void RunServiceContainersHealthcheck_healthyServiceContainer_AssertSucceededTask()
        {
            //Arrange
            Setup();
            _dockerManager.Setup(x => x.DockerInspect(_ec.Object, It.IsAny<string>(), It.IsAny<string>(), true)).Returns(Task.FromResult(healthyDockerStatus));
        ...
        public async void RunServiceContainersHealthcheck_healthyServiceContainerWithoutHealthcheck_AssertSucceededTask()
        {
            //Arrange
            Setup();
            _dockerManager.Setup(x => x.DockerInspect(_ec.Object, It.IsAny<string>(), It.IsAny<string>(), true)).Returns(Task.FromResult(emptyDockerStatus));
